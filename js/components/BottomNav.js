/**
 * BottomNav v3 - Inline styles (100% reliable)
 * No dependency on CSS variables or external stylesheets
 */
...
export const BottomNav = {
  items: [
    ...
    },
    {
      id: "saved",
      label: "Saved",
      icon: `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg>`,
      route: "#/bookmarks",
      locked: true
    },
    {
      id: "more",
      label: "More",
      icon: `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="1.5"></circle><circle cx="19" cy="12" r="1.5"></circle><circle cx="5" cy="12" r="1.5"></circle></svg>`,
      route: "#/more"
    }
  ],
  ...
  render() {
    ...
    const navStyle = `
      ...
      box-shadow: 0 -4px 20px rgba(28, 62, 44, 0.08);
  ...
  renderItem(item) {
    const itemStyle = `
      ...
      border: none;
      cursor: pointer;
      color: ${item.locked ? "#94A3B8" : "#84968B"};
    ...
    return `
      <a 
        class="nav-item ${item.locked ? "locked" : ""}" 
        data-nav-id="${item.id}"
  ...
  updateActive() {
    ...
    nav.querySelectorAll(".nav-item").forEach((item) => {
      ...
      if (isActive) {
        ...
        if (iconWrapper) {
          iconWrapper.style.background = "rgba(28, 62, 44, 0.08)";
        }
...
function createRipple(el, e) {
  try {
    ...
    ripple.style.cssText = `
      ...
      width: ${size}px;
      height: ${size}px;
      background: radial-gradient(circle, rgba(28, 62, 44, 0.25) 0%, transparent 70%);
      border-radius: 50%;
      pointer-events: none;
      transform: scale(0);
      opacity: 1;
      animation: dsRipple 0.6s ease-out forwards;
    `;
