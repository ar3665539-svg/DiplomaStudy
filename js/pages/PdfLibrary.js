/**
 * PdfLibrary — DB-driven
 */

import { AppShell } from "../components/AppShell.js";
import { getSubjects, getPdfsBySubject, getSubjectsByAssignment } from "../services/api.js";
...
export async function renderPdfLibrary() {
  ...
  AppShell.updateHeader({
    title: "PDF Library", subtitle: "Books, Notes & Handouts",
  ...
  try {
    ...
    for (const sub of subjects) {
      try {
        ...
        pdfs.forEach((p) => allPdfs.push({ ...p, subjectName: sub.name, subjectId: sub.id }));
      } catch (e) {}
    }
  } catch (err) {}
  ...
  if (allPdfs.length === 0) {
    main.innerHTML = emptyState({
      icon: "📄", title: "কোনো PDF নেই",
      message: "Admin Panel থেকে PDF upload করলে এখানে দেখা যাবে।",
  ...
  const bySubject = {};
  allPdfs.forEach((p) => {
    if (!bySubject[p.subjectId]) bySubject[p.subjectId] = { subject: subjectsMap[p.subjectId] || { name: p.subjectName, icon: "📘" }, pdfs: [] };
    bySubject[p.subjectId].pdfs.push(p);
  });

  main.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;padding:14px 16px;background:linear-gradient(135deg, #FEE2E2, #FECACA);border:1px solid #FCA5A5;border-radius:16px;margin-bottom:18px;">
      ...
      <div style="flex:1;min-width:0;">
        <div style="font-size:14px;font-weight:800;color:#991B1B;">${allPdfs.length}টি PDF</div>
        <div style="font-size:11px;color:#B91C1C;font-weight:600;">${Object.keys(bySubject).length}টি subject-এ</div>
      </div>
    </div>
      ...
      ${Object.values(bySubject).map(({ subject, pdfs }) => `
        <div>
          <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:linear-gradient(135deg, rgba(28,62,44,0.06), transparent);border-left:3px solid #1C3E2C;border-radius:10px;margin-bottom:10px;">
            ...
            <span style="font-size:10.5px;font-weight:800;color:#991B1B;background:#FEE2E2;padding:3px 9px;border-radius:999px;">${pdfs.length}</span>
          </div>
          <div style="display:flex;flex-direction:column;gap:10px;">
            ${pdfs.map((pdf) => `
              <a href="${pdf.fileUrl || '#'}" target="_blank" rel="noopener" ${pdf.fileUrl ? "" : `onclick="event.preventDefault(); alert('File নেই');"`} style="display:flex;align-items:center;gap:12px;padding:14px;background:#FFFFFF;border:1.5px solid #E1E8E1;border-radius:14px;text-decoration:none;cursor:pointer;box-shadow:0 2px 6px rgba(28,62,44,0.04);">
                <div style="width:48px;height:48px;border-radius:13px;background:linear-gradient(135deg,#FEE2E2,#FECACA);color:#991B1B;display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0;">📄</div>
                <div style="flex:1;min-width:0;">
